...
#### Debug Mode
After averaging the run times, the averages for the Assembly kernel in 2^20, 24, and 30 was calculated to be 0.70ms, 13.47ms, and 901.93ms respectively. Meanwhile, the averages for the C kernel were 2.0ms, 32.57ms, and 2301.67ms respectively. The percent differences between the averages are also shown to provide more context on how much faster and slower one kernel was over the other. Note that the original values of the averages were used to calculate for the percent values.
